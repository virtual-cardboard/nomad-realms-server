...
public class JoinClusterHttpHandler extends HttpEventHandler<JoinClusterRequestEvent, JoinClusterResponseEvent> {
	...
	public JoinClusterResponseEvent handle(JoinClusterRequestEvent request, PacketAddress clientAddress) {
		...
		if (data.getCluster(worldId) == null) {
			data.addCluster(new NetworkCluster(new WorldInfo(0, "This is a test", 12345, 1655063005817L, currentTimeMillis())));
		}
		...
		return new JoinClusterResponseEvent(spawnTime, spawnTick, nonce, username, peerLanAddresses, peerWanAddresses, spawnPos, idRange, cluster.worldInfo());
	}
